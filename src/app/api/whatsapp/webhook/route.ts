...
export async function POST(request: NextRequest) {
    ...
    console.log(`[Webhook WhatsApp] Chamada recebida. ID: ${requestId}`);

    try {
        // Tentar logar hit inicial, mas não deixar quebrar se o banco falhar
        try {
            ...
            console.error(`[Webhook] Erro ao salvar log inicial:`, dbError);
        }
        ...
        try {
            ...
            if (incomingApiKey !== expectedApiKey) {
                console.warn(`[Webhook] API Key mismatch! Recebido: ${incomingApiKey?.substring(0, 5)}...`);
                // Por enquanto, logamos o erro mas permitimos passar se for vindo de fontes conhecidas para debug
        ...
        console.error('Erro FATAL não tratado no webhook WhatsApp:', fatalError);
